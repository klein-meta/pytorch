...
def forward(self, x, y):
    ...
    def test_allow_complex_guards_as_runtime_asserts(self):
        # check allow_complex_guards_as_runtime_asserts flag behaves correctly
        ...
        for _strict in [True, False]:  # patching doesn't work for private export API
            ep = torch.export._trace._export(
                Mod4Reshape(),
                inputs,
                dynamic_shapes={"x": (dx, dy)},
    ...
    def test_allow_complex_guards_as_runtime_asserts_errors(self):
        # check error messages with allow_complex_guards_as_runtime_asserts = False/True
        class Foo(torch.nn.Module):
        ...
        with self.assertRaisesRegex(  # if allow = True, suggested fixes should not specialize.
            torch._dynamo.exc.UserError,
    ...
    def test_allow_explicit_guards_as_runtime_asserts(self):
        ...
        # check unbacked symint + not equal guards
        class Foo(torch.nn.Module):
            def forward(self, x, y):
                n = y.item()
                if x.shape[0] != n * 2:
        ...
        inputs = (torch.randn(10), torch.tensor(4))
        dynamic_shapes = {"x": [Dim("dx")], "y": None}
        for _strict in [True, False]:  # patching doesn't work for private export API
            ep = torch.export._trace._export(
                Foo(),
                inputs,
            ...
            ep.module()(torch.randn(8), torch.tensor(5))
            with self.assertRaisesRegex(
                RuntimeError,
                r"Runtime assertion failed for expression Ne\(s0 \- 2\*u[\d], 0\)",  # strict/non-strict difference in u0/u1
            ):  # fail only at runtime
                ep.module()(torch.randn(12), torch.tensor(6))  # fail
